...
MVC structuring, API calls, form entry, and dynamic layout are all successfully integrated together, bringing the basic demo to life! Try it for yourself! Just clone and open the .sln on main branch (the stable release), and run the VS project (http on localhost).
...
## Improvements:
In `AddNFTs`, the NFT actually isn't "saved". To actually be saved, what could further be coded is integrating Entity migrations, to store the actual API NFTList model data into a local relational DB. Then, when an NFT is "saved", it could be written to the NFTs table (based on the NFT.cs object model). Where the NFTGallery view would then display all NFTs from the local DB (what was originally queried, plus any the user adds afterwards)

Better styling could be applied too. And some elements like the buttons are not universally styled due to time constraints, but the `title` and `subtitle` elements on each page are common, to demonstrate some styling logic.
